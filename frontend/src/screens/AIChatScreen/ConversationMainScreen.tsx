...
export function ConversationMainScreen() {
  ...
  //  const [sendAdvisorAIMessage] = useSendProjectAdvisorMutation();
    ...
    } =  useListProjectConversationByIdQuery(sessionIdParam, {
    ...
    const handleProjectAIMessage = async (message: string) => {
        ...
        try {
            ...
            const response = await chatProjectAdvisor(session).unwrap();
    ...
    const handleProposalDraftAIMessage = async (messageContent: string) => {
    ...
    const handleAdvisorAIMessage = async (message: string) => {
        if (!message.trim()) return;

        if (!projectData?.sessionId) {
            toast.error("Conversation session was not found")
            return;
        }
        ...
        // Confirm if sessionId exist within ConversationMessage
        console.log(projectData.sessionId);
      ...
      //  appendMessage(userMessage);
        ...
        try {
            const payload = {sessionId: userMessage.sessionId,
                            messageContent: message
            };

            console.log("Advisor request payload:", payload);

            const response: any = await chatProjectAdvisor(payload).unwrap();
